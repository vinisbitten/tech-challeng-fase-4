...
def generate_pdf_report(analysis: dict, output_path: str):
    ...
    story.append(Paragraph(f"<b>Arquivo:</b> {analysis['video']}", styles["Normal"]))
    story.append(Paragraph(f"<b>Analisado em:</b> {analysis['analyzed_at']}", styles["Normal"]))
    story.append(Paragraph(f"<b>Duracao:</b> {analysis['duration_seconds']}s", styles["Normal"]))
    story.append(Paragraph(f"<b>Nivel de Risco:</b> {analysis['risk_level']}", styles["Normal"]))
    story.append(Spacer(1, 0.5 * cm))

    story.append(Paragraph("<b>Deteccoes por Categoria</b>", styles["Heading2"]))
    det_data = [["Categoria", "Ocorrencias"]] + [
        [k, str(v)] for k, v in analysis["detections_by_class"].items()
    ]
    if len(det_data) > 1:
